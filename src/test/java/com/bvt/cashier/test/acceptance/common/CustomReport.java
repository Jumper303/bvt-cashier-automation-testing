...
public class CustomReport implements IReporter {
	...
	//TODO:  application version, application configurations, links to screenshot, summary pie chart
	protected void writeSuiteSummary() {
		...
		writer.print("<table><tr><th>Test Environment</th><th>Browser</th><th>Application version</th></tr>");
		writer.print("<tr><td>" + TestFixture.getTargetEnvironment() + "</td><td>" + TestFixture.getBrowser() + "</td><td>Application version</td></tr></table>");
		...
		if (!classResults.isEmpty()) {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					if (aResult.getStatus() == ITestResult.FAILURE) {
						screenshotTag = "<td rowspan=\"1\"> <a href=\"" + aResult.getInstanceName() + "-" + aResult.getName() + "_failed.jpg\">screenshot</a></td>"; 
					}
					// Write the timing information with the first scenario per method
					buffer.append("<td><a href=\"#m").append(scenarioIndex).append("\">").append(methodName + "</a></td>").append(screenshotTag).append("<td rowspan=\"").append(resultsCount).append("\">").append(parseUnixTimeToTimeOfDay(start)).append("</td>").append("<td rowspan=\"").append(resultsCount).append("\">").append(decimalFormat.format(millisecondsToSeconds(duration))).append("</td></tr>");
					scenarioIndex++;

					// Write the remaining scenarios for the method
					for (int i = 1; i < resultsCount; i++) {
						buffer.append("<tr class=\"").append(cssClass).append("\">").append("<td><a href=\"#m").append(scenarioIndex).append("\">").append(methodName + "</a></td>").append("").append("</tr>");
						scenarioIndex++;
					}
	...
	private XmlInclude getMethod(ITestResult result) {
		XmlInclude currentMethod = null;
		Iterator testsIterator = result.getTestContext().getCurrentXmlTest().getSuite().getTests().iterator();

		outer_cycle_label: while (testsIterator.hasNext()) {
			XmlTest test = (XmlTest) testsIterator.next();
			if (test.getName().equals(result.getTestContext().getName())) {
				Iterator classIterator = test.getXmlClasses().iterator();
				while (classIterator.hasNext()) {
					XmlClass xmlClass = (XmlClass) classIterator.next();
					if (xmlClass.getName().equals(result.getInstanceName())) {
						Iterator methodIterator = xmlClass.getIncludedMethods().iterator();
						while (methodIterator.hasNext()) {
							XmlInclude method = (XmlInclude) methodIterator.next();
							if (method.getName().equals(result.getMethod().getMethodName())) {
								currentMethod = method;
								break outer_cycle_label;
							}
						}
					}
				}
	...
	private void writeScenario(int scenarioIndex, String label, ITestResult result) {
		...
		XmlInclude method = getMethod(result);
		if (method != null) {
			...
			while (keyIterator.hasNext()) {
				...
				writer.print(keyIterator.next().toString());
				writer.print("</th>");
			}
			...
			while (keyIterator.hasNext()) {
				writer.print("<td>");
				writer.print(Utils.escapeHtml(Utils.toString(parameters.get(keyIterator.next()), Object.class)));
				writer.print("</td>");
			}
			writer.print("</tr>");
